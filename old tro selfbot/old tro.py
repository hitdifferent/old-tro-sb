class SELFBOT():
    ...
    # Dont just skid it, gimme some credits, thank you - 39#1337
...
ctypes.windll.kernel32.SetConsoleTitleW(f'[TROGANISTANS v{SELFBOT.__version__}] | Loading...')
...
def startprint():
  ...
  ╚══██╔══██╔══████╔═══████║    ████╔══██████╗  ████╔════╝
   ...
   ╚═╝  ╚═╝  ╚═╝╚═════╝╚══════╚═╚═╝  ╚═╚═╝  ╚═══╚══════╝
                       ...
                       {Fore.CYAN}TRO'GANISTANS {SELFBOT.__version__} | {Fore.GREEN}Logged in as: {Alucard.user.name}#{Alucard.user.discriminator} {Fore.CYAN}| ID: {Fore.GREEN}{Alucard.user.id}   
                       ...
                       {Fore.CYAN}Creator(Private For TRO'GANISTANS): {Fore.GREEN}39#1337
    '''+Fore.RESET)
...
def Init():
    if config.get('token') == "token-here":
        ...
        try:
            ...
            os.system(f'title (TROGANISTANS) - Version {SELFBOT.__version__}')
...
async def on_connect():
    ...
    startprint()
    ctypes.windll.kernel32.SetConsoleTitleW(f'[TRO GANISTANS v{SELFBOT.__version__}] | Logged in as {Alucard.user.name}')
...
async def help(ctx):
 ...
 embed.set_author(name="Prefix='13'", icon_url="https://cdn.discordapp.com/attachments/702281004311904267/726113814751215666/image0_1.gif")
 embed.set_image(url="https://cdn.discordapp.com/attachments/702281004311904267/726120903724761170/image0.gif")

 embed.add_field(name="__**> Tro Old Selfbot**__", value="this bot was developed by hoeable#1337, for help, contact them on discord.", inline=True)
...
async def wizz(ctx):
    ...
    show_avatar.set_image(url='https://cdn.discordapp.com/attachments/702281004311904267/726120903724761170/image0.gif')
    ...
    for i in range(1, 25):
              await ctx.guild.create_text_channel(name=f'DONT FUCK WITH TRO {i}')
              await ctx.guild.create_voice_channel(name=f'unhoeable was here {i}')
              await ctx.guild.create_category(name=f'tro was here {i}')
...
async def banall(ctx):
    await ctx.message.delete()
    await ctx.send("***unhoeable was here***")
    show_avatar = discord.Embed(
...
@Alucard.command(name='groupleaver', aliase=['leaveallgroups', 'leavegroup', 'leavegroups'])
